...
use crate::types::{AppSettings, AuditLogEntry, FetchLogsParams, LogsResponse};
...
/// Download and parse the rotated log file for a given `LogRecord`
/// straight from IPFS, so an auditor can inspect individual entries.
#[tauri::command]
pub async fn fetch_log_entries(
    state: State<'_, AppState>,
    cid: String,
) -> Result<Vec<AuditLogEntry>, ClientError> {
    let gateway = state.settings.read().await.ipfs_gateway_base_url.clone();
...
/// in-memory state so subsequent fetches use the new values.
#[tauri::command]
pub async fn save_settings(
    ...
    let value = serde_json::to_value(&settings).map_err(|e| ClientError::Store(e.to_string()))?;
