...
use super::constants::{ALS_ENDPOINT, ALS_QUEUE_DIR};

const QUEUE_FILE: &str = "als_queue.jsonl";
...
impl AlsQueue {
    fn file_path() -> PathBuf {
        PathBuf::from(ALS_QUEUE_DIR).join(QUEUE_FILE)
    }

    pub async fn push(entry: QueueEntry) -> Result<(), String> {
    ...
    pub async fn read_all() -> Vec<QueueEntry> {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            match serde_json::from_str::<QueueEntry>(&line) {
                ...
                Err(e) => eprintln!("[ALS Queue] skip baris rusak: {e}"),
            }
        }
    ...
    pub async fn rewrite(remaining: &[QueueEntry]) -> Result<(), String> {
        let file_path = Self::file_path();
...
pub fn spawn_retry_worker() {
    // println!("[ALS] Retry Woker telah ke Spawn");
    tokio::spawn(async move {
        let client = reqwest::Client::new();
        let mut consecutive_failures: u32 = 0;

        loop {
            let entries = AlsQueue::read_all().await;
            ...
            println!(
                "[ALS Worker] {} entry pending, mencoba kirim...",
                entries.len()
            );
            ...
            for mut entry in entries {
                if entry.attempt_count >= MAX_ATTEMPTS_BEFORE_SKIP {
                    eprintln!(
                        "[ALS Worker] entry {} sudah {} kali gagal, skip (label: {})",
                ...
                match result {
                    Ok(res) if res.status().is_success() => {
                        println!(
                            "[ALS Worker] ✓ entry {} berhasil (label: {}, attempt: {})",
                    ...
                    Ok(res) => {
                        eprintln!(
                            "[ALS Worker] ✗ entry {} gagal — server {}: {} (label: {})",
                            entry.id,
                            res.status(),
                    ...
                    Err(e) => {
                        eprintln!(
                            "[ALS Worker] ✗ entry {} gagal kirim: {e:?} (label: {})",
            ...
            if any_success {
                if let Err(e) = AlsQueue::rewrite(&remaining).await {
                    eprintln!("[ALS Worker] gagal rewrite queue: {e}");
                }
                consecutive_failures = 0;
            }
